...
from config import Architecture, Config, URLs
from exceptions import DependencyError, GadgetError
...
class GadgetManager:
    ...
    def __init__(self, logger: Logger, config: Config):
        ...
        self.session.headers.update({"User-Agent": "FridAPK/1.0"})

    def get_frida_version(self) -> str:
        """Get installed Frida version"""
        try:
            result = subprocess.run(
                ["frida", "--version"], capture_output=True, text=True, check=True
            )
                ...
                for chunk in response.iter_content(chunk_size=8192):
                    if chunk:
                        ...
                        if total_size > 0:
                            ...
                            self.logger.info(
                                f"Downloading {target_path.name} - {progress: 03d}%"
                            )
    ...
    def find_gadget_for_architecture(self, arch: str) -> Optional[Path]:
        """Find appropriate gadget for given architecture"""
        for gadget_file in self.config.gadgets_dir.glob("*.so"):
            if arch in gadget_file.name.lower():
    ...
    def get_recommended_gadget(self) -> Optional[Path]:
        ...
        available_gadgets = list(self.config.gadgets_dir.glob("*.so"))
        self.logger.info(
            f"Found {len(available_gadgets)} gadgets in {self.config.gadgets_dir}"
        )

        if not available_gadgets:
            self.logger.error(
                "No Frida gadgets found! Gadgets should be pre-installed in the image."
            )
            self.logger.error(f"Expected location: {self.config.gadgets_dir}")
            return None

        # Try ARM64 first (most common)
        gadget = self.find_gadget_for_architecture("arm64")
        if gadget:
            return gadget

        # Fallback to ARM
        gadget = self.find_gadget_for_architecture("arm")
        if gadget:
            return gadget

        # Use first available as last resort
        self.logger.info(f"Using first available gadget: {available_gadgets[0].name}")
        return available_gadgets[0]
