...
class APKProcessor:
    ...
    def extract_apk(self, apk_path: Path, destination: Path, extract_resources: bool = True) -> None:
        ...
        try:
            ...
            if not extract_resources:
                cmd.append('-r')
                self.logger.info(f'Extracting {apk_path.name} (without resources) to {destination}')
            else:
                self.logger.info(f'Extracting {apk_path.name} (with resources) to {destination}')
                self.logger.info('Some errors may occur while decoding resources with framework dependencies')
    ...
    def repackage_apk(self, source_dir: Path, output_path: Path, use_aapt2: bool = False) -> Path:
    ...
    def _generate_keystore(self, keystore_path: Path) -> None:
        ...
        cmd = [
            ...
            '-validity', '700', '-noprompt',
            '-alias', 'apkpatcheralias1',
            '-dname', 'CN=apk.patcher.com, OU=ID, O=APK, L=Patcher, S=Patch, C=BR',
    ...
    def _sign_v1(self, apk_path: Path, keystore_path: Path) -> None:
        ...
        cmd = [
            ...
            '-keystore', str(keystore_path), '-storepass', 'password',
            str(apk_path), 'apkpatcheralias1'
        ]
